...
public class RESTfulCommunicator {
	...
	public static void checkLoginCredentials(final String email,
			final String password, final AsyncResponseHandler handler) {
		AsyncHttpClient client = new AsyncHttpClient();
		client.setCookieStore(cookieStore);
		client.post(BASE_URL + "api/login", new RequestParams("email", email,
				"password", password), new AsyncHttpResponseHandler() {
	...
	/**
	 * Post request to send Locations of the user to the server
	 */
	public static void postLocation(Context context, final Location location,
		...
		client.post(context, BASE_URL + resource, se, "application/json",
				new AsyncHttpResponseHandler() {
					public void onSuccess(String response) {
						if (response.equals("true")) {
							if (handler != null) {
								SessionManager.SESSION
										.updateLocationsArray(location);
								handler.onSuccess();
							}
						} else {
							if (handler != null) {
	...
	// SessionManager.SESSION.getCookieStore());
	// HttpResponse response = httpclient.execute(httppost, httpContext);
	 ...
	 */
	public static void createSession(final Long startDate, final Long endDate,
			final ArrayList<Guardian> guardians) {
		Thread t = new Thread() {
	...
	public static void getSession() {
		Thread t = new Thread() {
			public void run() {
				...
				// Getting session ID from shared preferences when you have
				// logged in and you reopen the app
	...
	public static void deleteSession() {
		Thread t = new Thread() {
			public void run() {
				...
				HttpClient httpclient = new DefaultHttpClient();
				HttpPost httppost = new HttpPost(BASE_URL + resource
						+ SessionManager.SESSION.getSessionId());
	...
	/**
	 * Panic alert to server
	 * @return
	 */
	public static void alertPanic(final AsyncResponseHandler handler) {
		AsyncHttpClient client = new AsyncHttpClient();
		client.setCookieStore(SessionManager.SESSION.getCookieStore());
		
		//Resource
		String resource = "api/alertPanic/";
		String sessionID = SessionManager.SESSION.getSessionId();
		resource += sessionID;
		Log.d("Alerting Panic: ", "Panic Button Pressed");
		client.post(BASE_URL + resource, new AsyncHttpResponseHandler() {
			public void onSuccess(String response) {
				Log.d("Response: ", response);
				if (response.equals("true")) {
					Log.d("Panic Alerted?", "Yes");
					handler.onSuccess();
				}
